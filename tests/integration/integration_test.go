...
func TestTodoAPP_Chat(t *testing.T) {
	...
	conversationSummaryRepo, err := depend.Resolve[domain.ConversationSummaryRepository]()
	require.NoError(t, err, "failed to resolve conversation summary repository")
	...
	t.Run("chat-todo-event-subscriber-generates-conversation-summary", func(t *testing.T) {
		require.Eventually(t, func() bool {
			summary, found, getErr := conversationSummaryRepo.GetConversationSummary(
				t.Context(),
			...
			return strings.TrimSpace(summary.CurrentStateSummary) != ""
		}, 2*time.Minute, 2*time.Second, "expected chat summary generated by ChatEventSubscriber")
	})
