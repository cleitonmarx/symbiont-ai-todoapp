...
description: Delete existing todos safely after resolving the right targets.
...
1.1. A plain-text "would delete" list is not completion; completion requires a successful `delete_todos` call.
...
7.1. Never claim deletion happened unless the tool result confirms deletion.
7.2. If deletion still fails, report failure clearly and ask only the minimum follow-up needed to retry.
