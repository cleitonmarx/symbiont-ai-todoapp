...
const (
	// MAX_CHAT_MESSAGES_FOR_TITLE is the number of recent messages considered for title generation.
...
func (gct GenerateConversationTitleImpl) Execute(ctx context.Context, event outbox.ChatMessageEvent) error {
	...
	if event.Type != outbox.EventType_CHAT_MESSAGE_SENT {
		return core.NewValidationErr("invalid event type for conversation title generation")
	}
	...
	resp, err := gct.assistant.RunTurnSync(spanCtx, assistant.TurnRequest{
		Model:       gct.model,
	...
	if applyStatus != assistant.ConversationTitleApplyStatus_Updated {
		rawTitle := strings.TrimSpace(resp.Content)
		switch applyStatus {
		case assistant.ConversationTitleApplyStatus_SkippedNotGrounded:
		...
		case assistant.ConversationTitleApplyStatus_SkippedEmpty, assistant.ConversationTitleApplyStatus_SkippedUnchanged:
...
func (gct GenerateConversationTitleImpl) queueTitleUpdate(ctx context.Context, conversation assistant.Conversation) {
...
func formatMessagesForConversationTitle(messages []assistant.ChatMessage) string {
	lines := make([]string, 0, len(messages))
	for _, message := range messages {
		if message.ChatRole != assistant.ChatRole_User && message.ChatRole != assistant.ChatRole_Assistant {
			continue
		}
...
func summarizeMessageForTitlePrompt(role assistant.ChatRole, content string) string {
	...
	if role == assistant.ChatRole_Assistant {
